...
class SettingsCog(commands.Cog):
    ...
    async def prefix(self, ctx: commands.Context, *args: str) -> None:
        ...
        if args:
            ...
            await ctx.send(
                ...
                format(prefix=prefix, syntax=syntax)
            )
    ...
    async def enable(self, ctx: commands.Context, *args: str) -> None:
        ...
        syntax = f'{prefix}enabled/disable <alert>'
        ...
        for arg in args:
            ...
            if alert in strings.ALERTS:
                ...
                updated_alerts[alert_column] = True if action == 'enable' else False
            else:
                ignored_alerts.append(alert)

        message_result = ''
        if updated_alerts:
            await database.update_guild(ctx, **updated_alerts)
            message_updated_alerts = (
                f'{action.capitalize()}d the following alerts:'
            )
            for alert in updated_alerts.keys():
                message_updated_alerts = f'{message_updated_alerts}\n{emojis.BP} `{alert}`'
            message_result = message_updated_alerts
        if ignored_alerts:
            message_ignored_alerts = (
                f'Could not find alerts with the following names:'
            )
    ...
    async def event_role(self, ctx: commands.Context, *args: str) -> None:
        ...
        if role_id not in (0, ctx.guild.id):
            ...
            if role is None:
                await ctx.reply(f'Invalid role.\n{message_syntax}', mention_author=False)
                return
        alerts = strings.ALERTS if arg_alert == 'all' else [arg_alert,]
        ...
        for alert in alerts:
            ...
            if alert in strings.ALERTS:
                ...
                updated_alerts[alert_column] = role_id
            else:
                ignored_alerts.append(alert)
        ...
        if updated_alerts:
            await database.update_guild(ctx, **updated_alerts)
            message_updated_alerts = (
                f'Updated the role for the following alerts:'
            )
            for alert in updated_alerts.keys():
                message_updated_alerts = f'{message_updated_alerts}\n{emojis.BP} `{alert}`'
            message_result = message_updated_alerts
    ...
    async def event_message(self, ctx: commands.Context, *args: str) -> None:
        ...
        for alert in alerts:
            ...
            if alert in strings.ALERTS:
                ...
                updated_alerts[alert_column] = message
            else:
                ignored_alerts.append(alert)
        ...
        if updated_alerts:
            await database.update_guild(ctx, **updated_alerts)
            message_updated_alerts = (
                f'Updated the message for the following alerts:'
            )
            for alert in updated_alerts.keys():
                message_updated_alerts = f'{message_updated_alerts}\n{emojis.BP} `{alert}`'
            message_result = message_updated_alerts
...
async def embed_guild_settings(bot: commands.Bot, ctx: commands.Context) -> discord.Embed:
    ...
    auto_flex_channel = ctx.guild.get_channel(guild_settings.auto_flex_channel_id)
    auto_flex_field = (
        f'{emojis.BP} Alerts: `{auto_flex_enabled}`\n'
        f'{emojis.BP} Channel: `{auto_flex_channel.name}`'
    )
